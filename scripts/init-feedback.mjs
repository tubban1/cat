...
try {
  const result = await pool.query(
    ...
    "to_regclass('public.cat_adaptive_players') is not null as adaptive_players_ready, " +
    "to_regclass('public.cat_adaptive_arms') is not null as adaptive_arms_ready, " +
    "to_regclass('public.cat_adaptive_updates') is not null as adaptive_updates_ready"
  );
  const row = result.rows[0];
  if (!row.feedback_ready || !row.events_ready || !row.runs_ready || !row.experiments_ready || !row.profiles_ready ||
      !row.adaptive_players_ready || !row.adaptive_arms_ready || !row.adaptive_updates_ready) {
